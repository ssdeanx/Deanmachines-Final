...
export function HeroSection() {
  ...
  return (
    <section className="relative isolate pt-14 pb-20 overflow-hidden card-membrane bg-[var(--color-background)]/80 backdrop-blur-lg">
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-25 -z-10" aria-hidden="true">
        <defs>
          <linearGradient id="hero-fiber" x1="0" y1="0" x2="100%" y2="100%">
            <stop offset="0%" stopColor="var(--color-accent)" />
            <stop offset="100%" stopColor="var(--color-primary)" />
          </linearGradient>
        </defs>
        <path d="M0,80 Q200,160 400,80 T800,80" fill="none" stroke="url(#hero-fiber)" strokeWidth="14" opacity="0.13" />
        <path d="M0,160 Q200,240 400,160 T800,160" fill="none" stroke="url(#hero-fiber)" strokeWidth="8" opacity="0.09" />
      </svg>
      ...
      <div className="pointer-events-none absolute inset-0 z-[-1]">
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          >
            ...
            >
              ...
              <span className="relative inline-block">
                <span className="bg-gradient-to-r from-[var(--color-accent)] via-[var(--color-primary)] to-[var(--color-foreground)] bg-clip-text text-transparent">
                  Deanmachines
                </span>
                <motion.span
                  className="absolute -right-6 -top-2 text-[var(--color-accent)]"
                  initial={{ opacity: 0, scale: 0 }}
                  animate={{ opacity: 1, scale: 1 }}
                  transition={{ delay: 1, duration: 0.4 }}
                >
                  <Sparkles className="h-6 w-6 text-[var(--color-accent)] drop-shadow-[0_0_8px_var(--color-accent)] animate-pulse" />
                </motion.span>
              </span>
            </motion.h1>
          ...
          >
            <div
              className="relative m-2 rounded-xl card-membrane p-2 lg:m-4 lg:rounded-2xl lg:p-4 group perspective border border-[var(--color-border)]"
              style={{
            ...
            >
              <div className="relative rounded-lg card-membrane shadow-2xl p-8 transform-gpu">
                {/* Glassmorphism glow effects */}
                <div className="absolute -left-4 -top-4 w-24 h-24 bg-[var(--color-accent)]/30 rounded-full blur-xl opacity-70 group-hover:opacity-100 transition-opacity duration-700" />
                <div className="absolute -right-4 -bottom-4 w-32 h-32 bg-[var(--color-primary)]/30 rounded-full blur-xl opacity-70 group-hover:opacity-100 transition-opacity duration-700" />
